...
export default function WorkHeroVideo() {
    ...
    const checkAndInit = () => {
      if (window.Vimeo && iframeRef.current && !player) {
        try {
          ...
          // Monitor time to loop back to 7 seconds when video ends
          const handleTimeUpdate = async (data: { seconds: number; duration: number }) => {
            // If video is within 0.5 seconds of the end, loop back to 7 seconds
            if (data.duration - data.seconds < 0.5) {
              vimeoPlayer.setCurrentTime(START_TIME).catch(() => {});
            }
          };
  ...
  // Build Vimeo URL with parameters to hide branding
  // Note: Loop is handled manually to start from 7 seconds, not URL parameter
  const vimeoUrl = `https://player.vimeo.com/video/${VIDEO_ID}?autoplay=1&muted=1&title=0&byline=0&portrait=0&badge=0&controls=0`;
